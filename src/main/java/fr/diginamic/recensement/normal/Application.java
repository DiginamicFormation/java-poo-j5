...
public class Application {
	...
	public static void main(String[] args) {
		...
		System.out.println("---------------------------------------------------");
		System.out.println("Affichage des 10 régions les plus peuplés de France");
		...
		for (int i=0; i<villes.size(); i++){
			...
			// On recherche la région correspondant à la ville dans la map
			Region region = mapRegions.get(codeRegion);
			
			// Si la région précédente n'existe pas, on la créée et on la stocke
			// dans la map
			if (region==null){
				region = new Region(codeRegion, v.getNomRegion());
				mapRegions.put(codeRegion, region);
			}
			
			// Ensuite on ajoute la population de la ville à la population du département
			region.setPopulation(region.getPopulation()+v.getPopulation());
		}
		
		// Une fois la création de la Map terminée, on récupère toutes les régions
		// et on les tri.
		...
		for (int i=0; i<villes.size(); i++){
			...
			// On recherche le département stocké dans la HashMap avec ce code
			Departement dept = mapDepts.get(codeDept);
